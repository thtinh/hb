...
class JACommentModelComments extends JModel {
	...
	function _getVars() {
		...
		//$list ['limit'] = $mainframe->getUserStateFromRequest ( $option . '.limit', 'limit', 10, 'int' );
		$list ['limit'] = 10;
	...
	function deleteComment($id) {
		...
		if($db->query()){
			...
			if ($items->userid) {					
				$title = sprintf(JText::_('JOMSOCIAL_ACTIVITI_STREAM_TITLE_REMOVE'), $items->referer, $items->contenttitle);
	...
	function store($post=0, $insert=0) {
		...
		if (!$insert) {
			...
			if (! isset($post["id"])) {					
				...
				$title = sprintf(JText::_('JOMSOCIAL_ACTIVITI_STREAM_TITLE_NEW_ITEM'), $row->referer, $row->contenttitle);
				...
				if(JACommentHelpers::isSpecialUser()){
					...
					$title = sprintf(JText::_('JOMSOCIAL_ACTIVITI_STREAM_TITLE_UPDATED_ITEM'), $row->referer, $row->contenttitle);
					JACommentHelpers::JomSocial_addActivityStream($user->id, $title, $row->id, $action);
				}				
			}			
		}
