...
$GLOBALS ['JACVERSION'] = '1.0.2';
$GLOBALS ['JACPRODUCTKEY'] = 'COM_JACOMMENT';
...
class JACommentHelpers { 
	...
	function getRealLengthOfComment($comment){
		$tags = array ('/\[LARGE\]/isUs','/\[\/LARGE\]/iUs',						
						'/\[MEDIUM\]/isUs','/\[\/MEDIUM\]/iUs',
			      		'/\[HR\]/iUs',
	      			    '/\[B\]/isUs','/\[\/B\]/iUs',
	      			    '/\[I\]/isUs','/\[\/I\]/iUs',
	      			    '/\[U]/isUs','/\[\/U\]/iUs',
	      				...
	      				'/\[SUB\]/isUs','/\[\/SUB\]/iUs',
	      				'/\[SUP\]/isUs','/\[\/SUP\]/iUs',
	      				'/\[QUOTE]/isUs','/\[\/QUOTE\]/iUs',
	      				'/\[LINK=(.*)\]/isUs','/\[\/LINK\]/iUs',
	     				'/\[IMG\]/isUs','/\[\/IMG\]/iUs',
	     				'/\[YOUTUBE\]/isUs','/\[\/YOUTUBE\]/iUs'
	    			  );	
	...
	function displayNote($message,$type){
		...
		jQuery(document).ready( function($) {
			...
			if(coo==1)
				...
				$('#jac_help').html('<?php echo JText::_('[Close]')?>');
		});	
		</script>		
		<?php 	
	}
	...
	function check_access() {
		...
		if ($access <= $aid) {
			...
			if (! $aid) {				
				echo '<div style="border-top:2px solid #dddddd;parding:10px 0 0 0;">';
				if(JPluginHelper::isEnabled('system', 'plg_jarpxnow')){					
					echo '<div id="jac-login-form" style="margin:20px auto;padding:20px;width:400px;">';
					echo '<h4>'. $jacconfig['general']->get('display_message') .'</h4>';
					echo '{jarpxnow}';
					echo '</div>';	
				}else{
				...
				echo '<div style="border-top:2px solid #dddddd;parding:10px 0 0 0;">';
				echo '<h4 style="width:100%; background-color:red;color:white;">'. $jacconfig['general']->get('display_message', JText::_('This site is down for maintenance. Please check back again soon.')) .'</h4>';
				echo '</div>';								
				return false;
			}
		}
		...
		if(!$user->guest){
			...
			JRequest::setVar("option", "com_jacomment");				
				echo '<div id="jac-wrapper"><div id="jac-login-form" style="margin:0px auto;width:400px;">';									
					echo JText::_("Please login to view comment.");						
	...
	function isSpecialUser($userID = 0, $action = '') {
		...
		// Check to see if the user has access to view the full article
		//$aid = $user->get ( 'aid' );				
		if(($user->usertype == "Super Administrator") || ($user->usertype == "Manager") || ($user->usertype == "Administrator")){
			return true;
		}else{
			//if pass userid, don't get aid because i use usertypy to check						
...
//        }
		...
		if($type=="addNew"){			
			...
			if($itemSendMails){
				...
				foreach ($itemSendMails as $itemSendMail){																					
					...
					$filters ['{CONFIG.SITE_TITLE}']  	 = $mainframe->getCfg('sitename');					
	...
	function replaceBBCodeToHTML($text){
		global $jacconfig;
		if($jacconfig["layout"]->get("enable_bbcode", 0)){		
			...
			if (class_exists ( 'DCODE' )) {			
				...
				$text = $myDcode->parse ($text, $jacconfig["spamfilters"]->get("is_nofollow"));
			}
		}else{
			$text = str_replace ("\n", "<br />", trim ($text));
		}
	...
	function sendMailWhenChangeType($userName, $userEmail, $content, $url="", $type, $action=''){
		global $jacconfig, $mainframe;
		$config = new JConfig();
		$tmpUrl = $mainframe->isAdmin() ? $mainframe->getSiteURL() : JURI::base();
		$url = $tmpUrl.$url;
		if($action){
			if($action == "removeSpam"){
				...
				$filters ['{MOD_REASONS}']  		 = JText::_("After consideration, we find that your comment has absolutely valid contents.");
	...
	function sendAddNewMail($commentID,$wherejatotalcomment='', $type='', $post=''){
		...
		if($jacconfig["general"]->get("is_enabled_email", 0)){			
			...
			if($jacconfig['general']->get("is_notify_admin", 1)){				
				$mail = $this->getEmailTemplate ( "Jacommentnotifying_admin_on_a_new_comment_posted" );
    ...
    function getCss($theme = '', $isFilePath = false)
    {
        ...
        $cssUrl = ($isFilePath ? JPATH_SITE : JURI::root()) . 'templates/' . $mainframe->getTemplate() . '/html/com_jacomment/'.$theme . '/css/style.css';        
        
        $cssPath = JPATH_SITE . DS . 'templates' . DS . $mainframe->getTemplate() . DS . '/html/com_jacomment/' . DS . $theme . DS . 'css' . DS . 'style.css';
        if (!is_file($cssPath)) {
...
//                                </object></p>';
    ...
    function showComment($str, $showYoutube = true){    	
        $comment = $this->showYoutube($this->showSmiley($str), $showYoutube);        
        return $comment;
    }
    ...
    function get_Authinfo($data, $iscurl) {
        if ($iscurl) {
            ...
            $json = file_get_contents ( "https://rpxnow.com/api/v2/auth_info/?token=" . $data ['token'] . "&apiKey=" . $data ['apiKey'] . "&format=json" );
        }
    ...
    function getAvatar($userID=0){
    	...
    	if(isset($jacconfig['layout']) && !$jacconfig['layout']->get('enable_avatar')){
			return $avatar;
		}
        ...
        if(!$avatar){					
	   		switch($jacconfig['layout']->get('type_avatar')){
			    case 1:
			    	if($this->checkComponent('com_comprofiler'))
			        	$avatar = $this->getAvatarCB($userID);
			        break;								
				case 2:
					if($this->checkComponent('com_fireboard'))
				...
				case 4:
				    ...
				    break;			    
				case 3:					
				    $avatar= $this->getAvatarGravatar($user->email, $jacconfig['layout']->get('avatar_size', 1), $defaultAvatar);
				    break;			
			}								
        }
...
class JACommentLicense
{
	...
	function verify_license() {
		...
		if (! function_exists ( 'curl_version' )) {			
			if (! ini_get ( 'allow_url_fopen' )) {
				...
				$result = $this->socket_getdata ( $req );
			}
		} else {						
		...
		if (! $result) { //Not connected to server
			...
			//$mainframe->redirect ( 'index.php?option=com_jacomment' );
		} else {
					
			switch ($result) {
				case md5 ( 'invalid_domain' ) :
					{
						$this->updateFail();
						JError::raiseWarning ( 1, JText::_ ( 'Your domain is not accepted. Therefore the system will be disabled. Please contact' ) . ' <a href="http://joomlart.com">JoomlArt</a> '.JText::_('for further assistance') );
						return;
					}
					break;
				...
				case md5 ( 'payment_not_completed' ) :
					{
						$this->updateFail();
				...
				case md5('success'):
					{
						$this->updateSuccess($payment_id, $email);						
						$mainframe->redirect('index.php?option=com_jacomment&view=comment&layout=license');
					}
					break;
				case md5('error'):
				default :
	...
	function updateFail() {				
		...
		$_SESSION['JATOOLBAR_VERIFY_PASSED'] = 0;
	}
	
	function updateSuccess($payment_id, $email) {
		$db = &JFactory::getDBO();
