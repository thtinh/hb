...
//        //echo '<a class="jac_links" href="'.$links.'#jacomments" title="'.JText::_('Add a jacomment').'">'.JText::_('Add a jacomment').'</a>';
//	}
//    
//	function getList() {
...
//    }
	...
	function downloadfile(){
		...
		if ($filename == ''){
			echo "<html><head><title>Downloads</title></head><body>".JText::_('ERROR: download file NOT SPECIFIED.')."</body></html>";
			exit;
		}
	...
	function uploadFileEdit(){
		...
		if (isset($_FILES['myfileedit']['name']) && $_FILES['myfileedit']['size']>0 && $_FILES['myfileedit']['size']<= $maxSize && $_FILES['myfileedit']['tmp_name']!=''){			
			if($this->checkFileUpload($_FILES, "edit")){					
				...
				if (count($listFiles)<$jacconfig['comments']->get("total_attach_file")) {														
					...
					foreach ($listFiles as $listFile){												
						...
						$img .= "<div style='float: left; clear: both;'><span><input type='checkbox' onclick='checkTotalFileEdit()' name='listfile[]' value='$listFile' checked></span>&nbsp;&nbsp;<img src='components/com_jacomment/themes/" . $theme . "/images/". $type .".gif' alt='". $type ."' /> " .$listFile . "</div>"; 				
					}
					//load file uncheck
					...
					foreach ($listFilesInFolders as $listFilesInFolder){
						if(!in_array($listFilesInFolder, $listFiles)){
							...
							$img .= "<div style='float: left; clear: both;'><span><input type='checkbox' onclick='checkTotalFileEdit()' name='listfile[]' value='$listFilesInFolder' disabled='disabled'></span>&nbsp;&nbsp;<img src='components/com_jacomment/themes/" . $theme . "/images/". $type .".gif' alt='". $type ."' /> " .$listFilesInFolder . "</div>";
						}
					}
					...
					elseif(@move_uploaded_file($_FILES['myfileedit']['tmp_name'], $target_path)){
						 ...
						 $img .= "<div style='float: left; clear: both;'><span><input type='checkbox' onclick='checkTotalFileEdit()' name='listfile[]' value='$fname' checked></span>&nbsp;&nbsp;<img src='components/com_jacomment/themes/". $theme ."/images/". $type .".gif' alt='". $type ."' /> " .$fname . "</div>";
				   }
				}
				...
				function stopUpload(par, listfile, count, totalUpload){					  		  
						  ...
						  if(eval(count)>=totalUpload){
						  		if(1<=totalUpload){
									...
									par.getElementById(\'err_myfileedit\').innerHTML = "'. JText::_("You added ").'" + totalUpload + " '. JText::_("file").'!";
						  		}else{						  		
						  			par.form1edit.myfileedit.disabled = true;
									par.getElementById(\'err_myfileedit\').innerHTML = "'. JText::_("You added ").'" + totalUpload + " '. JText::_("files").'!";
								}  																
						  }
						  return true;   
				}</script>';
				if($fileexist){				
					echo '<script language="javascript" type="text/javascript">								
							var par = window.parent.document;		
							par.getElementById(\'jac_upload_processedit\').style.display=\'none\';											
							par.getElementById(\'err_myfileedit\').innerHTML = "<span class=\'err\' style=\'color:red\'>'. JText::_("This file existed.") .'</span>";									
							par.getElementById("jac_upload_processedit").style.display="none";
						  </script>';														
				}else{
				...
				$strTypeFile = JText::_("Support file type: ").$attachFileTypes." ".JText::_("only");
				echo '<script language="javascript" type="text/javascript">
							var par = window.parent.document;
		...
		}elseif (isset($_FILES['myfileedit']['name'])){
			echo '<script type="text/javascript">					
					...
					par.getElementById(\'jac_upload_processedit\').style.display=\'none\';
					par.form1edit.myfileedit.value = "";
					par.getElementById(\'err_myfileedit\').innerHTML = "'.JText::_("Limitation of upload is ").$helper->getSizeUploadFile().'.";  		
	...
	function uploadFile(){
		...
		if (isset($_FILES['myfile']['name']) && $_FILES['myfile']['size']>0 && $_FILES['myfile']['size']<= $maxSize && $_FILES['myfile']['tmp_name']!=''){						
			...
			if($this->checkFileUpload($_FILES)){
				...
				if ($numberOfFile <$jacconfig['comments']->get("total_attach_file")) {
					...
					foreach ($listFiles as $listFile){												
						...
						$_SESSION['jaccount'] += 1;										 
						$img .= "<div style='float: left; clear: both;'><span><input type='checkbox' onclick='checkTotalFile()' name='listfile[]' value='$listFile' checked></span>&nbsp;&nbsp;<img src='components/com_jacomment/themes/" . $theme . "/images/". $type .".gif' alt='". $type ."' /> " .$listFile . "</div>"; 				
					}
					//load file uncheck
					$listFilesInFolders  = JFolder::files($destination_path);					
					foreach ($listFilesInFolders as $listFilesInFolder){
						if(!in_array($listFilesInFolder, $listFiles)){
							...
							$img .= "<div style='float: left; clear: both;'><span><input type='checkbox' onclick='checkTotalFile()' name='listfile[]' value='$listFilesInFolder' disabled='disabled'></span>&nbsp;&nbsp;<img src='components/com_jacomment/themes/" . $theme . "/images/". $type .".gif' alt='". $type ."' /> " .$listFilesInFolder . "</div>";
						}
					}
					...
					elseif(@move_uploaded_file($_FILES['myfile']['tmp_name'], $target_path)){
						...
						$_SESSION['jaccount'] += 1;
						$img .= "<div style='float: left; clear: both;'><span><input type='checkbox' onclick='checkTotalFile()' name='listfile[]' value='$fname' checked></span>&nbsp;&nbsp;<img src='components/com_jacomment/themes/" . $theme . "/images/". $type .".gif' alt='". $type ."' /> " .$fname . "</div>";
				   }
				}								
				...
				function stopUpload(par, listfile, count, totalUpload){					  		  
						  ...
						  if(eval(count)>=totalUpload){
						  		if(1<=totalUpload){
									...
									par.getElementById(\'err_myfile\').innerHTML = "'. JText::_("You added ").'" + totalUpload + " '. JText::_("file").'!";
						  		}else{
						  			par.form1.myfile.disabled = true;
									par.getElementById(\'err_myfile\').innerHTML = "'. JText::_("You added ").'" + totalUpload + " '. JText::_("files").'!";
						  		}
				...
				if($fileexist){
					echo '<script language="javascript" type="text/javascript">							
							var par = window.parent.document;
							par.getElementById(\'jac_upload_process\').style.display=\'none\';
							par.getElementById(\'err_myfile\').innerHTML = "<span class=\'err\' style=\'color:red\'>'. JText::_("This file existed.") .'</span>";
							par.getElementById("jac_upload_process").style.display="none";
						  </script>';										
				}else{			   
			...
			echo '<script type="text/javascript">
					...
					par.form1.myfile.value = "";
					par.getElementById(\'err_myfile\').innerHTML = "'.JText::_("Limitation of upload is ").' '.$helper->getSizeUploadFile().'!";  		
	...
	function resultChangeType(&$k, &$object, $id, $action = ''){
		...
		//enable button reply
			...
			$object [$k]->id = '#jac-span-reply-'.$id;
			$object [$k]->type = 'html';						
			$object [$k]->content = '<a id="jac-a-reply-'.$id.'" href="javascript:replyComment('.$id.',\''.JText::_("Posting").'\',\''.JText::_("Reply").'\')" title="'.JText::_("Reply comment").'"><span id="reply-'.$id.'">'.JText::_("Reply").'</span></a>';			
			$k++;
			
			$object [$k] = new stdClass ( );
			$object [$k]->id = '#jac-div-quote-'.$id;
			$object [$k]->type = 'html';									
			$object [$k]->content = '<a id="jac-a-quote-'.$id.'" href="javascript:replyComment(\''.$id.'\',\''.JText::_("Quoting").'\',\''.JText::_("Quote").'\',\'quote\')" title="'.JText::_("Quote this comment and reply").'"><span id="quote-'.$id.'">'.JText::_("Quote").'</span></a>';
			$k++;			
		}
	}
	...
	function deleteComment(){
		...
		if($id != ''){								 							
			...
			if(count($result)>0){				
				...
				$object [$k]->id = '#jac-msg-succesfull';
				$object [$k]->type = 'html';
				$object [$k]->content = $message.JText::_("You can't delete comment because it has sub comment!");
				$k ++;								
			}else{									
				//delete comment
				...
				$helper->displayInform(JText::_("Comment was deleted!"), $k, $object);								
...
//	}
	...
	function resultTextVote($id, $avatarSize, $voted){
		...
		$txt = '<span class="vote-comment-'.$avatarSize.' '.$jacVoteClass.'" id="voted-of-'.$id.'">'.$totalVote.'&nbsp;<strong>'.JText::_("Thanks for your vote!").'</strong></span>';		
		return $txt;
	}
	
	function voteComment() {
		global $mainframe, $jacconfig;
		...
		if (! $currentUserInfo->guest) {						
			...
			if ($typeVote == 1) {									
				...
				if (! $logs || $logs->votes == 0) {
					...
					if (! $modelLogs->store ( $post )) {
						$helper->displayInform(JText::_ ( "Error occurred! Data not saved" ), $k, $object);							
						...
						$helper->displayInform(JText::_ ( "Thanks for your vote!" ), $k, $object);														
					}
				} else {
					$helper->displayInform(JText::_ ( "You already voted for this comment!" ), $k, $object);						
				}
			} 
			...
			if ($allowGuestVoting == "all") {								
				...
				if($typeVote == "1"){
					...
					if ($voted) {
						$helper->displayInform(JText::_ ( "You already voted for this comment today!" ), $k, $object);					
					} else {
	...
	function saveEditComment(){		
		...
		if($post["comment"] == "" || $lengthOfComment == 0){
			$helper->displayInform(JText::_ ( "You must input your comment!" ).$messEnableButton, $k, $object);
			$validateData = 0;							
		}else if($lengthOfComment < $jacconfig['spamfilters']->get("min_length", 0)){
			if($validateData)
				$helper->displayInform(JText::_ ( "Your comment is too short!" ).$messEnableButton, $k, $object);
			$validateData = 0;							
		}else if($lengthOfComment > $jacconfig['spamfilters']->get("max_length", 500)){
			if($validateData)						
				$helper->displayInform(JText::_ ( "Your comment is too long!" ).$messEnableButton, $k, $object);
			$validateData = 0;									    					
		}else{				
			...
			if($model->checkMaxLink($post['comment'], $jacconfig['spamfilters']->get("number_of_links", 5))){				
				if($validateData)
					$helper->displayInform(JText::_ ( "Number of links in the comment exceed its maximum!" ).$messEnableButton, $k, $object);
				$validateData = 0;														
			}			
		}	
		...
		if($validateData){	
			...
			switch ($checkComment){
				case "IP Blocked":
					$helper->displayInform(JText::_ ( "Your IP is blocked; therefore, you can't post comment!" ).$messEnableButton, $k, $object);
					break;
				case "Email Blocked":
					$helper->displayInform(JText::_ ( "Your Email is blocked; therefore, you can't post comment!" ).$messEnableButton, $k, $object);
					break;
				case "Word Blocked";
				 	$helper->displayInform(JText::_ ( "Your word in the comment is blocked; therefore, you can't post comment!" ).$messEnableButton, $k, $object);
					break;				
				default:
					$messageBlacklist = "";					
					...
					if(!$commentID){
						$helper->displayInform(JText::_ ( "Error occurred! Data not saved" ).$messEnableButton, $k, $object);		
					}else{
						...
						if($stringAttach){
							$stringAttach = "<div class='jac-list-upload-title'>".JText::_('List Upload File:')."</div><div class='jac-list-upload-title'>".$stringAttach."</div>";									 
						}
...
//					$post["comment"] = $helper ->replaceURLWithHTMLLinks($post["comment"]);
	...
	function addNewComment(){	   
		global $mainframe, $jacconfig, $session_auth;
		...
		if(!isset($jacconfig['spamfilters'])){
			$jacconfig['spamfilters'] = new JParameter('');
		}
		...
		if($currentTotal >= $jacconfig["comments"]->get("maximum_comment_in_item", 20)){
			$helper->displayInform(JText::_ ( "You can't post new comment because the comment in this article is full." ).$messEnableButton, $k, $object);
			$validateData = 0;
		}
				...
				if($jacconfig['spamfilters']->get("is_enable_captcha", 1)){
				    ...
				    if(!$this->validatecaptchaaddnew($captcha)){					
						if($validateData){
							...
							$object [$k]->type = 'html';
							$object [$k]->content = JText::_("Your captcha was invalid. Please try again.").$message.$messEnableButton;
							$k ++;
							$validateData = 0;
						}					
					}																						
		...
		if (! $currentUserInfo->guest) {				
            ...
            $params = new JParameter ( $currentUserInfo -> params );
            if(array_key_exists('providerName', $params->_registry['_default']['data'])){
                $usertype = $params->_registry['_default']['data']->providerName;
...
//								print_r($listFileTemp);die();																
					    ...
					    if($post['parentid']){					    	
							...
							if($numberOfChild == 1){
								...
								$object [$k]->type = 'html';
								$object [$k]->content = '<a href="Javascript:displayChild(\''. $post['parentid'] .'\')" title="'.JText::_("Show all children comment of this comment.").'" class="showreply-btn" id="a-show-childen-comment-of-'. $post['parentid'] .'" style="display: none;">'.JText::_('Show').'									
																<span id=\'jac-total-childen-'. $post['parentid'] .'\'>1 </span>'. JText::_("Reply").'  																	
														</a>
														<a href="Javascript:displayChild(\''. $post['parentid'] .'\')" title="'.JText::_("Hide all children comment of this comment.").'" class="hidereply-btn" id="a-hide-childen-comment-of-'. $post['parentid'] .'">'.JText::_('Hide').'									
																<span id=\'jac-total-childen-'. $post['parentid'] .'\'>1 </span>'. JText::_("Reply").'  																	
														</a>'.$script;
	...
	function resultReportComment($id){
		...
		$result = "<a href='javascript:undoReportComment(". $id .")' class='jac-undo-report' title='".JText::_("Undo report.")."'>[". JText::_("undo") ."]</a>";
		return $result;	
	}
	...
	function resultUndoReportComment($id){		
		global $jacconfig;
		//layout[button_type]		
		$result = '<a class="report-btn" href="javascript:reportComment("'. $id .'")" title="'.JText::_("Flagged! it will be removed when enough people flag it.").'">'. JText::_("Report") .'</a>';		
		return $result;	
	}
	...
	function undoReportComment(){		
		...
		if (! $currentUserInfo->guest) {
			...
			if ($isAllowReport) {				
				...
				$helper->displayInform(JText::_ ( "Don't allow report comment!" ), $k, $object);								
			}
		} //if user is guest
		else {
			//param guest voting	
	...
	function attachFile(){
		...
		if(isset($_FILES['userfile']) && $_FILES['userfile']['name']!=''){
			...
			if(JFile::upload($_FILES['userfile']['tmp_name'], $desk)){
				...
				$filter_lang = $mainframe->getUserStateFromRequest( $option.'.jaemail.filter_lang', 'filter_lang', 'en-GB',	'string' );	
				return $this->setRedirect( "index.php?option=$option&view=jaemail&filter_lang=$filter_lang", JText::_('Import success.') );
			}
			unset($_FILES['userfile']);
			JError::raiseWarning(1, JText::_('Upload file not success.'));
...
//        $message = '<script type="text/javascript" id="script_error">textCounter("newcomment", "jaCountText");</script>';
//        $object [$k] = new stdClass ( );
//        $object [$k]->id = '#jac-msg-succesfull';
//        $object [$k]->type = 'html';
//        $object [$k]->content = $message;
