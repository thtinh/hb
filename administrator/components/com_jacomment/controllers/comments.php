...
class jacommentControllercomments extends JACommentController
{
	...
	function display(){	    	
		...
		if ($user->id==0)
		{
			JError::raiseWarning(1001,JText::_("YOU MUST BE SIGNED IN"));
	...
	function save(&$errors){		
       ...
       // if (!$row->bind(JRequest::get('post')))
		...
		if (!$row->bind( $post )) 
		{
			$errors[]=JText::_("Do not bind data");
		...
		if($count>0){
			$errors[] = JText::_ ( "ERROR OCCURRED! DUPLICATE FOR COMMENT TITLE" );
			return FALSE;
		}		

		if (!$item->store())
		{	    	
			$errors[]=JText::_("ERROR OCCURRED! DATA NOT SAVED");
			return FALSE;
		}
	...
	function uploadFileReply(){
		...
		if (isset($_FILES['myfile']['name']) && $_FILES['myfile']['size']>0 && $_FILES['myfile']['size']<= $maxSize && $_FILES['myfile']['tmp_name']!=''){																										
			...
			function stopUpload(par, listfile, count, totalUpload){					  		  
					  ...
					  if(eval(count)>=totalUpload){
					  		if(1<=totalUpload){
								...
								par.getElementById(\'err_myfile_reply\').innerHTML = "'. JText::_("You added ").'" + totalUpload + " '. JText::_("file").'!";
					  		}else{						  		
					  			par.formreply.myfile.disabled = true;
								par.getElementById(\'err_myfile_reply\').innerHTML = "'. JText::_("You added ").'" + totalUpload + " '. JText::_("files").'!";
							}  																
					  }					  
					  return true;   
			}</script>';
			...
			if($fileexist){				
				echo '<script language="javascript" type="text/javascript">								
						var par = window.parent.document;													
						par.getElementById(\'err_myfile_reply\').innerHTML = "<span class=\'err\' style=\'color:red\'>'. JText::_("This file existed.") .'</span>";									
						par.getElementById("upload_process_1_reply").style.display="none";
					  </script>';														
			}else{
			...
			echo '<script type="text/javascript">					
					...
					par.getElementById(\'upload_process_1_reply\').style.display=\'none\';
					par.formreply.myfile.value = "";
					par.getElementById(\'err_myfile_reply\').innerHTML = "'. JText::_("Limitation of upload is ").$helper->getSizeUploadFile().'.";  		
		...
		if($validateData){			
			...
			if($jacconfig['comments']->get("is_attach_image", 0)){
				...
				if($listFile){
					...
					$strTitle = "<div><span>".JText::_('List Attach File:')."</span></div><div>";
				...
				if($parentID){
					...
					if (! $modelLogs->store ( $postLogs )) {
						//$this->displayInform(JText::_ ( "Error occurred! Data not saved" ), $k, $object);							
	...
	function publish()
	{
		...
		{
			JError::raiseWarning(1001, JText::_( 'Error occurred! Data not saved' ));
		} else {
			$msg = JText::_( 'Save data successfully' );
		}
	...
	function lock()
	{
		...
		if(!$model ->locked($id, $val))
		{
			...
			$msg = JText::_( 'LOCK DATA SUCCESSFULLY' );
		}
	...
	function remove(){
		...
		if($errors){
			...
			$msg=JText::_("Delete data successfully");
		$this->setRedirect('index.php?option=com_jacomment&view=comments',$msg);
	}
    ...
    function embed_youtube(){
        ...
        if (! $helper->checkYoutubeLink($post['txtYouTubeUrl'])) {
        	$this->displayInform(JText::_( "Youtube Video Url is incorrect" ), $k, $object);            
        }else{
        	$this->displayInform(JText::_( "VIDEO IS EMBED" ), $k, $object);            
