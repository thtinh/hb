...
class JACommentControllerManagelang extends JACommentController{
	...
	function save(){
		...
		if($this->getTask() == 'apply'){
			$this->setRedirect('index.php?option='.$option.'&view=managelang&task=edit&layout=form&client='.$post['client'].'&lang='.$post['filename'], JText::_('UPDATED LANGUAGE FILE SUCCESSFULLY'));
		}else{
			$this->setRedirect('index.php?option='.$option.'&view=managelang&client='.$post['client'], JText::_('UPDATED LANGUAGE FILE SUCCESSFULLY'));
		}
	}
}
?>