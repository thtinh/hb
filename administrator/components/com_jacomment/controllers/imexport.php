                ...
                if($out == false){
                	...
                	if($source == "jacomment"){
                		...
                		if($allComments[0]->name() != "comments"){
                			JError::raiseNotice(1, JText::_('Please select xml file of jacomment component.'));
	                   	 	return $this->setRedirect("index.php?option=com_jacomment&view=imexport&group=import");	 
                		}
...
//	                   	 	return $this->setRedirect("index.php?option=com_jacomment&view=imexport&group=import");
	                		...
	                		if(count($arrayExitsIDs) >0){	                			
	                			if(count($arrayExitsIDs) >1){
	                				...
	                				$strErrorID = JText::_("Can't import comment: ").$arrayExitsIDs[0]["id"].JText::_(". Because it already exists.")."<br />";	
	                			}
	                		...
	                		if(count($arrayExitsParentIDs) > 0){	                			
	                			if(count($arrayExitsParentIDs) >1){
	                				...
	                				$strErrorParentID = JText::_("Can't import comment: ").$arrayExitsParentIDs[0]["id"].JText::_(". Because we can't find parent of it in database.")."<br />";	
	                			}
                		...
                		if($source == "intensedebate"){
                			if($allComments[0]->name() != "blogpost"){
                				JError::raiseNotice(1, JText::_('Please select xml file of intensedebate comments.'));
	                   	 		return $this->setRedirect("index.php?option=com_jacomment&view=imexport&group=import");		 	
                			}
    ...
    function import(){
        ...
        if(!$model->importData($source, '', $rows)){
            JError::raiseNotice(1, JText::_('ERROR OCCURRED! CAN IMPORT THE DATA.'));
        }
    ...
    function getComponent(){
    	...
    	if(isset($result->name)){
    		if($result->name == "MyBlog"){    			
				...
				echo "---".JRoute::_($route->getArticleRoute($result->id.":".$result->title,$result->catID.":".$result->catTitle));	
    		}
    	}else{
    		echo "com_content";
    		require_once(JPATH_SITE.DS.'components'.DS.'com_content'.DS.'helpers'.DS.'route.php');
