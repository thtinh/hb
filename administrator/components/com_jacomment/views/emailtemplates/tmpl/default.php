...
<form name="adminForm" action="index.php" method="post">
	<fieldset class="adminform TopFieldset">
		<div id="comment-header-search-left">
			...
			<input type="button" onclick="document.getElementById('search').value='';this.form.getElementById('filter_catid').value='0';this.form.getElementById('filter_state').value='';this.form.submit();" value="<?php echo JText::_( 'Filter Reset' ); ?>">
		</div>
									...
									<?php if(!isset($items[$i]) && isset($en_items[$i])){?>
										<?php foreach ($en_items[$i] as $item){?>
											<tr>
												...
												<td class="col_2">
													<?php echo $item['title']; ?> 
													<a href="<?php echo JRoute::_( 'index.php?option='. $this->lists['option'] .'&view=emailtemplates&task=duplicate&filter_lang='.$this->filter_lang.'&cid='.$item['id'] ); ?>" onclick="return confirm('<?php echo JText::_('Are you sure you want to copy the template from this default template?')?>')" title="<?php echo JText::_('Email template does not exist, click here to copy it from the default file'); ?>"><img src="<?php echo JURI::base()?>components/com_jacomment/asset/images/copy-32x32.png" alt="Copy" /></a>
												</td>
												<td class="col_3">&nbsp;</td>
											</tr>
										<?php }?>
