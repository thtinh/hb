...
<form name="adminForm" id='adminForm<?php echo $this->curentTypeID; ?>-<?php echo $this->id; ?>' action="index.php" method="post"> 
	...
	<div id="jac-quick-reply" class="clearfix">			
		<h3><?php echo JText::_("Edit a comment");?></h3>
	    <div id="plugin_embed">
        <?php 
        ...
        <?php if($jacconfig['layout']->get('enable_youtube')==1){ ?>
            ...
            function open_youtube(id){
                jaCreatForm('open_youtube',id,340,200,0,0,'<?php echo JText::_("Embed a YouTube Video");?>',0,'<?php echo JText::_("Embed Video");?>');
            }
            </script>
            <a href="javascript:open_youtube(<?php echo $this->id;?>);" class="plugin"><img title="Add a YouTube Video" alt="YouTube" src="http://www.youtube.com/favicon.ico"/> <span><?php echo JText::_("Embed Video");?></span></a>
        <?php } ?>     
        <?php if($jacconfig['layout']->get('enable_after_the_deadline', 0)==1){ ?>          
            <a href="javascript:jac_check_atd('')"><img title="Proofread Comment After the Deadline" alt="AtD" src="http://www.polishmywriting.com/atd_jquery/images/atdbuttontr.gif"/> <span id="checkLink"><?php echo JText::_("Check Spelling"); ?></span></a>
        <?php } ?>
...
?>
	<form id="formreply" name="formreply" class="wufoo" autocomplete="off" enctype="multipart/form-data" method="post" action="index.php">						
		<div>
			<h4><?php echo JText::_("Attached file!");?></h4>
		</div>
		<div>
			...
			<div class="small" style="font-size:10px">(<?php echo JText::_("Total:");?><?php echo $totalAttachFile; ?> <?php if($totalAttachFile >1){ echo JText::_("files -Max size:").'<B>'.$helper->getSizeUploadFile().'</B>';}else{echo JText::_("file -Max size:").'<B>'.$helper->getSizeUploadFile().'</B>';}?>)</div>
...
<div style="width: 100%;" class="lst_btn clearfix">
		<div style="float: right;">
			<a href="javascript:cancelEditComment('<?php echo $this->id?>', '<?php echo $this->curentTypeID;?>')" class="btn btn_cancel"><?php echo JText::_("Cancel");?></a>
