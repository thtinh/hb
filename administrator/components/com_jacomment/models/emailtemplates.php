...
class JACommentModelEmailtemplates extends JModel
{
	...
	*/
    function getItems($filter_lang='', $type=false){    		
		...
		$filter_order		= $mainframe->getUserStateFromRequest( $option_1.'.filter_order',		'filter_order',		'name',	'cmd' );
	...
	*/
	function store(){
		...
		if (!$row->bind( $post )) {
			JError::raiseWarning(1, JText::_('ERROR OCCURRED! CAN NOT BIND THE DATA'));
			return false;
		}
		
		if (!$row->check())
		{
			JError::raiseWarning(1, JText::_('ERROR: Invalid datas'));
			return false;
		}
      	
		if (!$row->store())
		{
			JError::raiseWarning(1, JText::_('ERROR OCCURRED! DATA NOT SAVED'));
			return false;			
		}
	...
	function do_duplicate(){
		...
		if(!$cid){
			JError::raiseWarning(1, JText::_('NO TEMPLATE IS SELECTED'));
			return false;
		}
		...
		if($items && $filter_lang){							
			foreach ($items as $k=>$item){
				...
				if($overwrite || (!$overwrite && !$id)){				
					...
					JError::raiseNotice(1, JText::_('Email template has really exist.'). ' ID='.$id);
				}
			}
			
			return true;
		}
		JError::raiseWarning(1, JText::_('Not find email template for copty or you have not selected a language to copy'));
		return false;
	}
	...
	*/
	...
	function import($filecontent){
		...
		if(!$list){
			JError::raiseWarning(1, JText::_('Import failure! File is empty or data is not correct format.'));
			return false;
		}
		...
		foreach ($list as $item){
			if(isset($item['name']) && $item['name']!='' && $filter_lang!=''){
				...
				if($overwrite || (!$overwrite && !$id)){	
					...
					JError::raiseWarning(1, JText::_('Email template has really exist.'). ' ID='.$id);
					return false;
				}
