...
var FormCheck = new Class({
	options : {
		...
		display : {
			...
			errorsLocation : 3,				//1 : tips, 2 : before, 3 : after
	...
	*/
	removeError : function(obj) {
		this.firstError = false;
		this.alreadyIndicated = false;
		obj.errors = [];//		$('name').style.border = '1px solid #BBBBBB';//		$('email').style.border = '1px solid #BBBBBB';//		$('title').style.border = '1px solid #BBBBBB';//		$('description').style.border = '1px solid #BBBBBB';				obj.element.style.border = '1px solid #BBBBBB';
		obj.isOK = true;
	...
	*/
	...
		/*	Function: validateOnSubmit		Add or remove onSubmit	*/		validateOnSubmit : function(enable) {		if (enable == true)		{			this.form.addEvents({				"submit": this.onSubmit.bind(this)			});		}		else		{			this.form.removeEvents("submit");		}	},			
	/*
	Function: resetError
		Clear all error msg
	*/
	...
	resetError: function() {
	this.validations.each(function(el) {
			if(el.element) this.removeError(el);
		}, this);
	},			/*	Function: checkForm		Check form anytime, no need to wait until submit	*/		checkForm: function() {		this.reinitialize();//?				this.validations.each(function(el) {			if(!this.manageError(el,'submit')) this.form.isValid = false;		}, this);				var valid = this.form.isValid;		if (!valid)			this.focusOnError(this.firstError);		return valid;	},
