...
class modSponsorsHelper {
    function getList(&$params) {
        $model		= modSponsorsHelper::getModel();
    ...
    function renderBanner($params, &$item) {
        ...
        if (trim($item->custombannercode)) {
            ...
            $html = str_replace( '{NAME}', $item->name, $html );
             ...
             //in case the image's dimension is outside 150x100 constraint, it would destroy template layout
            $img_path = $baseurl.'images/banners/'.$item->imageurl;
            // Constraints
            $max_width = 150;
            $max_height = 100;
            ...
            $image 	= '<img width="'.$width.'" height="'.$height.'" class="SponsorToolTip"'. 'title="'.$item->name .'::'.$item->description.'" src="'.$img_path.'" alt="'.JText::_('Banner').'" />';
            if ($item->clickurl) {
